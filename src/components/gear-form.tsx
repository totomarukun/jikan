    ...
    <div className="mt-2 space-y-3 rounded-xl border border-tt-green/40 bg-tt-soft-green/40 p-3 text-sm">
      ...
      <div className="flex gap-2">
        <button
          onClick={submit}
          disabled={busy}
          className="flex-1 rounded-xl bg-tt-green py-2.5 font-bold text-white transition hover:opacity-90 active:scale-95 disabled:opacity-50"
        >
          {busy ? "追加中..." : "追加する"}
        </button>
        <button
      ...
      {picked && (
        <>
          ...
          <button
            onClick={submit}
            disabled={busy}
            className="w-full rounded-xl bg-tt-green py-3 font-bold text-white shadow-lg shadow-tt-green/20 transition hover:opacity-90 active:scale-95 disabled:opacity-50"
          >
